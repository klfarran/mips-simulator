	...
	int main() {
		...
		while(active == true) { 
			...
			while (!fin) {
				...
				fin.clear(); //clear error state before opening file
				fin.open(filename.data());
			}

		    string instruction;

		    //populate instruction memeory
		    while (fin >> instruction) {
			    instructions.push_back(convert_binary(instruction));
		    }
		  
		    //populate instruction memeory for the processor 
			processor.Set_Instr_Mem(instructions);
			
			//first tick -> kicks off the program currently loaded into instruction memory 
			processor.Tick();
		 
			//once we return, give option to load another program 
			cout << "Load another program? y/n" << endl; 
			string another;
			cin >> another; 
		 
			while(another != "y" || another != "n") {
			    cout << "Please enter either \"y\" for yes or \"n\" or no" << endl;
			    cin >> another; 
			}
	...
	/*
	 //while the PC is a valid address, run program  
	   while(PC >= 0 && PC < instructions.size()) { 
			...
			//pass instruction to the pipeline-starts with IF
			//IF(memory, Register_File, PC);
